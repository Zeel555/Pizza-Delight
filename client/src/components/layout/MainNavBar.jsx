...
function MainNavbar() {
  ...
  return (
    <>
    <nav className="fixed bg-green-900 w-screen flex flex-row items-center justify-between px-6 sm:px-16 shadow-sm space-x-3">
        <NavLink
          href="/"
          className="flex flex-row justify-center items-center text-green-400 font-bold text-lg sm:text-3xl"
        >
          <img
            src={Logo}
        ...
        <div className="items-center justify-center space-x-4 hidden md:inline-flex">
          {NavItems.map((navItem, index) => (
            <NavLink
              ...
              className={({ isActive }) => {
                if (isActive) {
                  return 'text-lg text-green-400 border-b-4 border-green-400';
                }
                return 'text-lg  text-white hover:text-green-400 hover:border-b-4 hover:border-green-400';
              }}
            >
              {navItem.name}
            </NavLink>
        ...
        <button
          type="button"
          className="text-black hover:text-green-400 focus:outline-none md:hidden"
      ...
      {isOpen && (
        <div className="fixed font-semibold inset-0 bg-gray-800 bg-opacity-90 flex items-center justify-center md:hidden transition-all duration-300 ease-in-out">
          ...
          <button
            type="button"
            className="absolute top-8 right-9 text-white hover:text-green-400 focus:outline-none  md:hidden"
            onClick={() => setIsOpen(!isOpen)}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
            ...
            {NavItems.map((navItem, index) => (
              <NavLink
                ...
                className={({ isActive }) => {
                  ...
                  return 'text-2xl text-white hover:text-green-400';
                }}
              >
                {navItem.name}
              </NavLink>
