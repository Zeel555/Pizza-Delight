...
function PaymentStep({ setCurrentStep }) {
  ...
  const submitHandler = (e) => {
    ...
    dispatch(createRazorPayOrder({ amount, currency: 'INR' }));
  };
  ...
  return (
    <form onSubmit={submitHandler} className="w-full p-4">
      <p className="text-center text-black text-xl leading-relaxed">
        Payment
        <br />
        <span className="text-sm text-green-600">Select Payment Method</span>
      </p>
      {loading ? (
        <Loader />
        ...
        <div className="flex flex-col items-center justify-center w-full border border-green-300 rounded-2xl p-4">
