...
import { FaCartPlus, FaMoneyCheckAlt, FaPizzaSlice } from 'react-icons/fa';
...
function HowItWorksSection() {
  ...
  return (
    <section
      id="how-it-works"
      className="min-h-screen flex flex-col justify-center items-center py-12 px-10 sm:px-16 bg-green-50"
    >
      <h1 className="text-3xl sm:text-4xl md:text-5xl text-green-600 font-bold mb-2">
        How It Works
      </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-white rounded-2xl shadow-lg p-8">
        {StepsToOrderPizza.map((step) => (
          ...
          >
            <div className="flex items-center justify-center w-16 h-16 bg-green-100 text-2xl text-green-600 rounded-full mb-4 lg:mr-4">
              {step.icon}
            </div>
            <div className="flex flex-col text-center lg:text-left ">
              <h3 className="text-xl font-semibold border-b-2 border-b-green-600 mb-2">
                {step.step}
                {'. '}
                {step.title}
              </h3>
