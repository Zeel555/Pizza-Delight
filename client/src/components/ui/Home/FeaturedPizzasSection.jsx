function FeaturedPizzasSection() {
  ...
  return (
    <section
      id="featured-pizzas"
      className="min-h-screen flex flex-col justify-center items-center py-16 sm:py-12 px-10 sm:px-16 bg-amber-50"
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {featuredPizzas.map((pizza) => (
          <div
            key={pizza.id}
            className="bg-green-50 rounded-2xl shadow-lg overflow-hidden hover:transform hover:scale-105 transition-transform duration-500 ease-in-out"
          >
            <img
              src={pizza.imageUrl}
              alt={pizza.name}
              className="w-full h-48 object-cover p-4 border-b border-green-200"
            />
